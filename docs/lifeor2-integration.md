...
7. Configure inference on the client server. The supplied development endpoint is `http://127.0.0.1:8000/v1`, model `gemma-4-12B-it-8bit`. Put a required provider key in `LLM_API_KEY` in the untracked app environment file. Do not put it in chat, a `NEXT_PUBLIC_*` variable, shell command arguments, or screenshots. Restart the Next.js process after changing secrets.
